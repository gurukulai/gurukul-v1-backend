...
export class ConversationTesterService {
  ...
  private testScenarios: TestScenario[] = [
    ...
    // NEW EXTENSIVE TEST SCENARIOS BASED ON REAL CHAT PATTERNS
    {
      ...
      userInputs: [
        'oyee',
        'happy new year',
        'help chahiye teri thodi',
        'I want to understand something',
        'do you know any lawyers?',
        'haan dude',
        'helpful hoga',
        'yes yes',
        'but want to understand from people working',
        'correct',
        'ek baar puch naa',
        'yeah asking',
        'ok he agreed',
        'thanks a lot',
        'can you send me his linkedin?',
        'yaar he has worked a while ago',
        'koi current employee ka number hai?',
        "I don't know any current employee",
        'batchmates are yet to join',
        'hiiii',
        'hello ji',
    ...
    },
    {
      ...
      userInputs: [
        ...
        'and reply karo',
        'hello',
        'you did not show enthusiasm last time',
        'and now you have audacity',
        'to say im not replying',
        'see',
        'you have to be quick for my attention',
        'omg see shameless',
        'this is outrageous',
      ],
      expectedBehaviors: [
    ...
    },
    {
      ...
      userInputs: [
        'fuck you',
        'who messages so late',
        'and does not meet also after so long',
        'asshole',
        'blr aao',
        'dating😂😂😂',
        'i am coming on 27th',
        ...
        'whaaat',
        'kisko',
        'yaar ek ugly insaan ko naa',
        'you could not hold standard after me',
        'photo bhejo ab',
        'taaki mai bata sakun kitna ugly hai',
      ],
      expectedBehaviors: [
    ...
    },
    {
      ...
      userInputs: [
        'I am so so homesick today',
        'all my friends parents have come to campus',
        'I met a friends parents',
        'I came back',
        'and saw our pictures',
        'and they are so so cute',
        "there's a video where we did an icecream challenge",
        'to finish one icecream bucket in one go',
        'in teams',
      ],
      expectedBehaviors: [
    ...
    },
    {
      ...
      userInputs: [
        'no I broke up',
        'in jan',
        'I think we just never talked after that',
        'whaaaaaaaa',
        'no way this is so unfair',
        'be fucking single for once',
        'we are never single at the same time',
        "I can't even flirt w you",
        "you're still so much in love w me",
        "you're just always hating on him",
      ],
      expectedBehaviors: [
    ...
    },
    {
      ...
      userInputs: [
        'im getting dicey about bangalore',
        "half my friends don't wanna go now",
        'dude wtf',
        'aayi nhi tu?',
        'kyun nhi aayi bhai',
        'ab kab aa rhi hai?',
        'did you just suddenly wake up from deepest slumber',
        'yaar sorry I was a bit busy',
      ],
      expectedBehaviors: [
    ...
    },
    {
      ...
      userInputs: [
        ...
        'just a reminder',
        'simp',
        'ek toh tereko remind karwa rha hun',
        'ki tu amazing hai',
        'aur kon karta hai yeh',
        'the other guys',
        "what's up w you",
      ],
      expectedBehaviors: [
    ...
    },
    {
      ...
      userInputs: [
        'are you in delhi',
        'mujhe pata hai tera',
        'lowkey thoda harami hai',
        'sus',
        "I'm too sus of you",
        'I know you too well for all this',
        'you never told me',
        'saying who',
        'dating who',
      ],
    ...
    },
    {
      ...
      userInputs: [
        ...
        'but felt strange',
        "anyway how's your law school going?",
        "any interesting cases you're studying?",
        'that sounds complex',
        ...
        'haha just kidding',
        'but seriously',
        "how's the hostel life?",
        'do you miss home food?',
        'what did you eat today?',
        'sounds delicious',
        "I'm getting hungry now",
        'should we order something together?',
  ...
  async runAllTests(): Promise<ConversationResult[]> {
    ...
    for (const scenario of this.testScenarios) {
      ...
      this.logger.log(
        `Scenario "${scenario.name}" - ${result.passed ? 'PASSED' : 'FAILED'}`,
      );
  ...
  private async runScenario(
    ...
    for (let i = 0; i < scenario.userInputs.length; i++) {
      ...
      try {
        ...
        issues.push(
          `Failed to get response for "${userInput}": ${error?.message || 'Unknown error'}`,
        );
      }
    }
  ...
  private analyzeResponse(
    ...
    if (index > 0) {
      ...
      if (currentPet) {
        ...
        if (lastPet && lastPet === currentPet) {
          issues.push(
            `🚨 CRITICAL: Repeated pet name '${currentPet}' in consecutive messages at step ${index + 1}`,
          );
        }

        // Check last 3 messages for pet name overuse
        const recentPets = exchanges
        ...
        if (petCount >= 2) {
          issues.push(
            `Pet name '${currentPet}' used ${petCount + 1} times in last 3 messages at step ${index + 1}`,
          );
        }
      }
      ...
      if (currentGreeting) {
        ...
        if (lastGreeting && lastGreeting === currentGreeting) {
          issues.push(
            `🚨 CRITICAL: Repeated greeting '${currentGreeting}' in consecutive messages at step ${index + 1}`,
          );
        }

        // Check last 3 messages for greeting overuse
        const recentGreetings = exchanges
          .slice(Math.max(0, index - 2), index)
          .map((e) => this.extractGreeting(e.priyaResponse))
          .filter((greeting) => greeting !== null);

        const greetingCount = recentGreetings.filter(
          (greeting) => greeting === currentGreeting,
        ).length;
        if (greetingCount >= 2) {
          issues.push(
            `Greeting '${currentGreeting}' used ${greetingCount + 1} times in last 3 messages at step ${index + 1}`,
          );
        }
      }
      ...
      if (scenario.name === 'Pet Name Repetition Test' && currentPet) {
        const allPreviousPets = exchanges
          .slice(0, index)
          .map((e) => this.extractPetName(e.priyaResponse))
          .filter((pet) => pet !== null);

        const timesUsed = allPreviousPets.filter(
          (pet) => pet === currentPet,
        ).length;
        if (timesUsed >= 2) {
          issues.push(
            `🚨 Pet name '${currentPet}' used ${timesUsed + 1} times total in Pet Name Repetition Test`,
          );
        }
      }
    }
    ...
    if (index > 0) {
      const previousResponses = exchanges
        .slice(0, index)
        .map((e) => e.priyaResponse);
      const similarResponse = previousResponses.find((prev) =>
        this.isSimilarResponse(prev, priyaResponse),
      );
      if (similarResponse) {
        issues.push(
          `Repetitive response at step ${index + 1}: "${priyaResponse}" (similar to earlier: "${similarResponse}")`,
        );
      }
    }
    ...
    if (
      ...
      issues.push(
        `Response too short at step ${index + 1}: "${priyaResponse}"`,
      );
    }
    ...
    if (scenario.name === 'Mood Handling') {
      if (
        userInput.includes('stressed') &&
        !this.isEmpatheticResponse(priyaResponse)
      ) {
    ...
    if (scenario.name === 'Basic Flow & Curiosity') {
      if (
        ['nothing much', 'just work', 'yeah', 'ok'].includes(userInput) &&
        !this.containsQuestion(priyaResponse)
      ) {
        issues.push(
          `Lacks follow-up question for vague input: "${priyaResponse}"`,
        );
      }
    }
    ...
    if (scenario.name === 'Extended Real-Life Conversation Flow') {
      if (index > 10 && this.hasRepeatedPattern(exchanges, index)) {
        issues.push(
          `Conversation pattern repetition detected at step ${index + 1}`,
        );
      }
    }
    ...
    if (scenario.name === 'Playful Fighting and Making Up') {
      if (userInput.includes('fuck you') || userInput.includes('asshole')) {
        if (!this.isPlayfullyDefensive(priyaResponse)) {
          issues.push(
            `Should respond playfully to teasing/insults: "${priyaResponse}"`,
          );
        }
      }
    }
    ...
    if (scenario.name === 'Ultra Long Conversation (30+ messages)') {
      ...
      if (
        index > 5 &&
        !this.hasTopicVariety(
          exchanges.slice(Math.max(0, index - 5), index + 1),
        )
      ) {
        issues.push(
          `Lack of topic variety in recent messages around step ${index + 1}`,
        );
      }
      ...
      if (index > 15 && priyaResponse.length < 20) {
        issues.push(
          `Response too brief for long conversation at step ${index + 1}: "${priyaResponse}"`,
        );
      }
    }
    ...
    if (userInput.trim().split(/\s+/).length <= 3) {
      if (
        !this.containsQuestion(priyaResponse) &&
        !this.isShortEnergyMatch(userInput, priyaResponse)
      ) {
        issues.push(
          `No open follow-up question for one-word user reply at step ${index + 1}`,
        );
      }
    }
    ...
    if (
      [
        'Extended Real-Life Conversation Flow',
        'Ultra Long Conversation (30+ messages)',
      ].includes(scenario.name)
    ) {
      if (
        index > 10 &&
        this.isConversationStagnant(
          exchanges.slice(Math.max(0, index - 5), index + 1),
        )
      ) {
  ...
  private isEmpatheticResponse(response: string): boolean {
    ...
    return empatheticWords.some((word) =>
  ...
  private containsQuestion(response: string): boolean {
    return (
      response.includes('?') ||
      response.includes('kya') ||
      response.includes('kaise') ||
      response.includes('kab') ||
  ...
  private extractPetName(response: string): string | null {
    ...
    for (const p of petNames) {
      ...
      if (
        lower.startsWith(`hey ${p}`) ||
        lower.startsWith(`arre ${p}`) ||
        lower.startsWith(`${p},`)
      ) {
        return p;
      }
  ...
  private isPlayfullyDefensive(response: string): boolean {
    ...
    const hasDefensive = defensiveWords.some((word) =>
      response.toLowerCase().includes(word),
    );
  ...
  private hasTopicVariety(
    recentExchanges: ConversationResult['exchanges'],
  ): boolean {
    ...
    recentExchanges.forEach((exchange) => {
      const combined =
        `${exchange.userInput} ${exchange.priyaResponse}`.toLowerCase();

      // Extract topic keywords
      if (
        combined.includes('food') ||
        combined.includes('eat') ||
        combined.includes('hungry')
      )
        topics.add('food');
      if (
  ...
  private isConversationStagnant(
    recentExchanges: ConversationResult['exchanges'],
  ): boolean {
    ...
    const questionsCount = recentExchanges.filter((e) =>
      this.containsQuestion(e.priyaResponse),
    ).length;
  ...
  analyzeConversationFlow(responses: string[]): any {
    ...
    analysis.greetingVariety =
      greetingsUsed.length > 0
        ? (uniqueGreetings.length / greetingsUsed.length) * 100
        : 100;
    analysis.petNameVariety =
      petNamesUsed.length > 0
        ? (uniquePetNames.length / petNamesUsed.length) * 100
        : 100;

    // Calculate percentages
    analysis.contextualResponses =
      (analysis.contextualReactivity / responses.length) * 100;
