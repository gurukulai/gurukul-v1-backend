...
export class ConversationFlowService {
  ...
  async generateStrategy(
    ...
    if (!process.env.OPENAI_API_KEY) {
      this.logger.warn('OPENAI_API_KEY missing – skipping flow strategy generation');
      return null;
    }

    try {
      const analysisPrompt = `You are a Conversation Flow Analyst for an AI girlfriend persona called Priya.\n\n` +
