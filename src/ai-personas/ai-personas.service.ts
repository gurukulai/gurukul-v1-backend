...
export class AiPersonasService {
  ...
  async getEnhancedResponse(
    ...
    const trainingExamples =
      this.trainingDataService.getRelevantTrainingExamples(
        userInput,
        personaType,
    ...
    if (context?.userId) {
      ...
      if (!isNaN(userIdNum)) {
        ...
        recentHistory = history
          .slice(-10)
          .map((msg) => `${msg.fromUser ? 'User' : 'Priya'}: ${msg.message}`);
      }
    }
    ...
    try {
      ...
      if (personaType === 'PRIYA' && recentHistory.length > 0) {
        ...
        if (lastPriyaMessage) {
          const lastResponse = lastPriyaMessage.replace('Priya: ', '');
          processedResponse = this.fixPetNameRepetition(
            processedResponse,
            lastResponse,
          );
        }
      ...
      if (personaType === 'PRIYA') {
        const messages =
          this.trainingDataService.breakIntoMessages(processedResponse);
        return {
  ...
  private extractPetName(response: string): string | null {
    ...
    for (const p of petNames) {
      if (
        ...
        lower.startsWith(`hi ${p}`) ||
        lower.startsWith(`yo ${p}`) ||
        lower.startsWith(`${p},`) ||
        lower.includes(` ${p}!`) ||
        lower.includes(` ${p},`) ||
        lower.includes(` ${p} `) ||
  ...
  private getAlternativeGreetings(lastGreeting: string): string[] {
    ...
    return allGreetings.filter(
      (g) => g.toLowerCase() !== lastGreeting.toLowerCase(),
    );
  }
  ...
  private getAlternativePetNames(lastPetName: string): string[] {
    ...
    return allPetNames.filter(
      (p) => p.toLowerCase() !== lastPetName.toLowerCase(),
    );
  }
  ...
  private fixPetNameRepetition(
    ...
    console.log(
      `🔧 POST-PROCESSING: Detected pet name repetition "${currentPetName}" - fixing...`,
    );
    ...
    if (alternatives.length > 0) {
      ...
      console.log(
        `✅ POST-PROCESSING: Replaced "${currentPetName}" with "${randomAlternative}"`,
      );
      return fixed;
    }
