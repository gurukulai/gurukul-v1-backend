...
async function testNaturalConversationFlow() {
  ...
  console.log(
    'Based on real chat analysis from Aarushi-Sarthak conversation\n',
  );
  ...
  const testScenarios = [
    ...
    {
      name: 'Making Plans',
      messages: ['Free tonight?', 'How about dinner?', 'Pick you up at 8?'],
    },
    {
  ...
  for (const scenario of testScenarios) {
    ...
    for (const message of scenario.messages) {
      ...
      const response = Array.isArray(result.message)
        ? result.message.join(' ')
        : result.message;
  ...
  console.log(
    `   • Contextual Responses: ${analysis.contextualResponses.toFixed(1)}%`,
  );
  console.log(
    `   • Natural Flow (no greeting): ${analysis.naturalFlow.toFixed(1)}%`,
  );
  ...
  console.log(`\n🔄 REPETITION ANALYSIS:`);
  console.log(
    `   • Consecutive Greeting Repetition: ${analysis.consecutiveGreetingRepetition}`,
  );
  console.log(
    `   • Consecutive Pet Name Repetition: ${analysis.consecutivePetNameRepetition}`,
  );
  ...
  console.log(`\n📝 PATTERN DETAILS:`);
  console.log(
    `   • Greetings Used: ${analysis.greetingsUsed.join(', ') || 'None'}`,
  );
  console.log(
    `   • Pet Names Used: ${analysis.petNamesUsed.join(', ') || 'None'}`,
  );
  console.log(
    `   • Unique Greetings: ${analysis.uniqueGreetings.join(', ') || 'None'}`,
  );
  console.log(
    `   • Unique Pet Names: ${analysis.uniquePetNames.join(', ') || 'None'}`,
  );
  ...
  if (!overallSuccess) {
    ...
    if (!successCriteria.naturalFlow) {
      console.log(
        `   • Increase direct responses without greetings (target: 50%+)`,
      );
    }
    if (!successCriteria.contextualResponses) {
      console.log(
        `   • Add more contextual reactions like "Arree", "Yaar", "Really?"`,
      );
    }
