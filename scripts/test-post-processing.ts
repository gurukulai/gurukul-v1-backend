...
async function testPostProcessingFilter() {
  ...
  console.log(
    '📱 Simulating conversation with potential pet name repetition...\n',
  );
  ...
  const message1 = Array.isArray(response1.message)
    ? response1.message.join(' ')
    : response1.message;
  ...
  const message2 = Array.isArray(response2.message)
    ? response2.message.join(' ')
    : response2.message;
  ...
  const message3 = Array.isArray(response3.message)
    ? response3.message.join(' ')
    : response3.message;
  ...
  const message4 = Array.isArray(response4.message)
    ? response4.message.join(' ')
    : response4.message;
  ...
  responses.forEach((response, index) => {
    ...
    if (petName && lastPetName && petName === lastPetName) {
      ...
      console.log(
        `⚠️  REPETITION DETECTED: "${petName}" repeated from previous response`,
      );
    } else if (petName) {
  ...
  console.log(
    `   • Post-processing filter status: ${consecutiveRepetitions === 0 ? '✅ WORKING' : '⚠️ NEEDS IMPROVEMENT'}`,
  );
