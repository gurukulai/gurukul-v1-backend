...
async function testGreetingVariety() {
  ...
  for (let i = 0; i < testInputs.length; i++) {
    ...
    try {
      ...
      const responseText = Array.isArray(response.message)
      ...
      console.log(
        `🏷️  Greeting: ${greeting || 'none'} | Pet name: ${petName || 'none'}\n`,
      );
    } catch (error: any) {
  ...
  for (let i = 1; i < greetings.length; i++) {
    if (greetings[i] !== 'none' && greetings[i] === greetings[i - 1]) {
      ...
      console.log(
        `🚨 CONSECUTIVE GREETING REPEAT: "${greetings[i]}" at positions ${i} and ${i + 1}`,
      );
    }
  }
  ...
  for (let i = 1; i < petNames.length; i++) {
    if (petNames[i] !== 'none' && petNames[i] === petNames[i - 1]) {
      ...
      console.log(
        `🚨 CONSECUTIVE PET NAME REPEAT: "${petNames[i]}" at positions ${i} and ${i + 1}`,
      );
    }
  }
  ...
  console.log(
    `- Greeting variety score: ${uniqueGreetings.size}/${Math.max(totalGreetings, 1)} = ${((uniqueGreetings.size / Math.max(totalGreetings, 1)) * 100).toFixed(1)}%`,
  );
  console.log(
    `- Consecutive greeting repetitions: ${consecutiveGreetingRepeats}`,
  );
  console.log(`- Consecutive pet name repetitions: ${consecutivePetRepeats}`);

  const passed =
    consecutiveGreetingRepeats === 0 && consecutivePetRepeats === 0;
