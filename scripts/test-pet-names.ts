...
async function testPetNames() {
  ...
  for (let i = 1; i < petNames.length; i++) {
    if (petNames[i] !== 'none' && petNames[i] === petNames[i - 1]) {
      consecutiveRepeats++;
      console.log(
        `🚨 CONSECUTIVE REPEAT: "${petNames[i]}" at positions ${i} and ${i + 1}`,
      );
    }
  }
  ...
  console.log(
    `- Variety score: ${uniquePetNames.size}/${totalPetNames} = ${((uniquePetNames.size / Math.max(totalPetNames, 1)) * 100).toFixed(1)}%`,
  );
